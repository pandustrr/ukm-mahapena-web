...
import { Plus, Edit3, Trash2, Loader, Calendar, FileText, Search, Filter, X, AlertTriangle } from "lucide-react";
...
function ManajemenProker() {
    ...
    useEffect(() => {
        if (searchTerm) {
            const filtered = proker.filter((item) =>
    ...
    const handleDelete = async () => {
        setIsDeleting(true);
        try {
            await axios.delete(`http://127.0.0.1:8000/api/admin/proker/${deleteTarget.id}`, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem("adminToken")}`,
                },
            });
            setProker((prev) => prev.filter((proker) => proker.id !== deleteTarget.id));
                        ...
                        <table className="w-full rounded-lg overflow-hidden">
                            <thead className="bg-gray-100">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left font-medium text-gray-700">Nama Program</th>
